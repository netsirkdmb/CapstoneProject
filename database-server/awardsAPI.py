...
def createAwardPDFandEmail(request, awardID):
    ...
    return {"Status": "Success", "AwardPDFdata": awardData}, 200
...
class AwardsList(Resource):
    ...
    # create a new award in the database and email pdf to user
    def post(self):
        ...
        try:
            ...
            awardID = int(app.conn.insert_id())
            ...
            data = {"awardID": awardID, "receiverID": awardReceiverID, "giverID": awardGiverID, "typeID": awardTypeID, "awardDate": awardDate}

            awardPDFMessage = createAwardPDFandEmail(request, awardID)

            return {"Status": "Success", "Data": [data], "AwardPDF": awardPDFMessage}, 200
...
class Award(Resource):
    ...
    # update award in database and email updated pdf to user, errors if the award does not exist in the database
...
class CreateAward(Resource):
    ...
    def get(self, awardID):
        try:
            result = createAwardPDFandEmail(request, awardID)
